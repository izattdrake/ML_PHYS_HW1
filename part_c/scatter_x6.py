...
ax.scatter(x6, y, label='True', color='black')
ax.scatter(x6, y_predicted, label='Predicted', color='dodgerblue')

ax.set_xlabel(r'Longitude')
ax.set_ylabel(r'House Price per Unit Area')
ax.set_xlim(x6.min(), x6.max())

ax.grid(True, alpha=0.3, linewidth=0.6)
ax.legend(handlelength=2.2, labelspacing=0.35, borderaxespad=0.2)
save_axes_centered(fig, ax, f"norm_multi-longitude-scatter.pdf")    
